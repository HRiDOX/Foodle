...
from .utils import detectUser,send_verification_email_user,send_verification_email_vendor
...
def registerUser(request):
    if request.user.is_authenticated:
        ...
        if form.is_valid():
            ...
            email_template = 'accounts/emails/account_verification_email_user.html'

            send_verification_email_user(request, user,mail_subject,email_template)
...
def registerVendor(request):
    if request.user.is_authenticated:
        messages.warning(request,'You Are Already logged in!')
        return redirect('myVendorAccount')
    elif request.method == 'POST':
        form = UserForm(request.POST)
        ...
        if form.is_valid() and v_form.is_valid:
            ...
            # Send Verifcation email
            mail_subject = 'Click Below To Active Your Account'
            email_template = 'accounts/emails/account_verification_email_vendor.html'


            send_verification_email_vendor(request, user, mail_subject,email_template)
...
def activateVendor(request, uidb64,token):
    #Activate the user by setting the is_active status to True
